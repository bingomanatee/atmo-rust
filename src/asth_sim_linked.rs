...
use h3o::CellIndex;
use rand::Rng;
use crate::constants::{AVG_STARTING_VOLUME_KM_3, CELL_JOULES_EQUILIBRIUM, CELL_JOULES_START, LEVEL_AMT, ANOMALY_SPAWN_CHANCE, ANOMALY_DECAY_RATE, ANOMALY_ENERGY_AMOUNT, ANOMALY_VOLUME_AMOUNT};
...
impl AsthSimLinked {
    pub fn new(config: ASLParams) -> AsthSimLinked {
        let mut sim = match RockStore::open(&config.store_path) {
            ...
            Err(e) => { panic!("failed to load store: {:?}", e) }
        };
        ...
        let init_config = CellsForPlanetArgs {
            ...
            on_cell: |cell| {
                ...
                sim.cells.insert(cell.id, Rc::new(RefCell::new(linked_cell)));
                cell
            },
            res: ASTH_RES,
    ...
    fn export_visualization(&self) {
        ...
        if let Err(_) = fs::create_dir_all(output_dir) {
            eprintln!("Warning: Could not create visualization directory {}", output_dir);
            return;
        }
        ...
        let cells_for_export: Vec<(CellIndex, AsthenosphereCell)> = self.cells
            .iter()
            .map(|(&cell_id, cell)| {
        ...
        match image.save(&filename) {
            ...
            Err(e) => eprintln!("    ❌ Failed to export visualization for step {}: {}", self.current_step, e),
        }
    }
    
    fn advance_to_next_step(&mut self) {
        // 1. Save each cell to disk using batch save
        let cells_to_save: Vec<AsthenosphereCell> = self.cells
            .values()
            .map(|cell| {
        ...
        let cell_ids_with_next: Vec<CellIndex> = self.cells
            .iter()
    ...
    pub fn cool_cells(&self) {
        let cool_rate = (CELL_JOULES_EQUILIBRIUM / CELL_JOULES_START).powf(0.7/ (self.steps as f64));
    ...
    fn level_between_neighbors(&self, cell_a: &Rc<RefCell<AsthenosphereCellLinked>>, cell_b: &Rc<RefCell<AsthenosphereCellLinked>>) {
        ...
        if volume_diff > 0.0 {
            ...
            // Smooth exponential scaling with gentler curve
            let exponential_factor = if relative_diff > 0.05 {
                // For differences > 5% of average volume, apply moderate exponential scaling
                let excess = relative_diff - 0.05;
                0.5 + excess.powf(1.3) // Gentler power and higher baseline
            } else {
                // For small differences, apply moderate leveling (50% instead of 10%)
                relative_diff * 0.5 + 0.3 // Higher baseline flow rate
            };
            
            // Calculate transfer amount with moderate exponential scaling
            let base_transfer = volume_diff * LEVEL_AMT;
            let transfer_amount = base_transfer * exponential_factor;
    ...
    pub fn process_anomalies(&self) {
        ...
        for (_id, cell) in &self.cells {
            ...
            if (current.cell.anomaly_energy.abs() > 1e-6 || current.cell.anomaly_volume.abs() > 1e-6) 
                ...
                if let Some(next_cell) = &current.next {
                    ...
                    next.cell.anomaly_energy = current.cell.anomaly_energy * (1.0 - ANOMALY_DECAY_RATE);
                    next.cell.anomaly_volume = current.cell.anomaly_volume * (1.0 - ANOMALY_DECAY_RATE);
    ...
    pub fn spawn_anomalies(&self) {
        ...
        if rng.random::<f64>() < ANOMALY_SPAWN_CHANCE {
            ...
            if let Some(&random_cell_id) = cell_indices.get((rng.random::<f64>() * cell_indices.len() as f64) as usize) {
    ...
    fn apply_anomaly_to_cell_and_neighbors(&self, target_cell: &Rc<RefCell<AsthenosphereCellLinked>>, rng: &mut impl Rng) {
        // PARADIGM: Read current cells, write to next cells
        ...
        if let Some(next_cell) = &current.next {
            ...
            let energy_sign = if rng.random::<bool>() { 1.0 } else { -1.0 };
            let volume_sign = if rng.random::<bool>() { 1.0 } else { -1.0 };
            
            next.cell.anomaly_energy += ANOMALY_ENERGY_AMOUNT * energy_sign;
            next.cell.anomaly_volume += ANOMALY_VOLUME_AMOUNT * volume_sign;
        }
        ...
        for neighbor_id in neighbors {
            if let Some(neighbor_cell) = self.cells.get(&neighbor_id) {
                ...
                if let Some(neighbor_next) = &neighbor_current.next {
                    ...
                    let neighbor_energy_sign = if rng.random::<bool>() { 1.0 } else { -1.0 };
                    let neighbor_volume_sign = if rng.random::<bool>() { 1.0 } else { -1.0 };
                    
                    neighbor_next_mut.cell.anomaly_energy += ANOMALY_ENERGY_AMOUNT * 0.5 * neighbor_energy_sign;
                    neighbor_next_mut.cell.anomaly_volume += ANOMALY_VOLUME_AMOUNT * 0.5 * neighbor_volume_sign;
                }
            }
        }
    }
    ...
    pub fn run_simulation(&mut self, steps: u64) {
        println!("🌍 Starting AsthSimLinked simulation for {} steps...", steps);
        ...
        if self.visualize {
            println!("🎉 Simulation completed! {} PNG files exported to vis/asth_sim_linked/", (steps / 5) + 1);
        } else {
...
mod tests {
    ...
    fn test_simulation_run_through() {
        ...
        // Verify final state
        assert!(sim.cells.len() > 0, "Expected cells to remain after simulation");
    ...
    fn test_level_cells() {
        ...
        if let (Some(cell_a), Some(cell_b)) = (cell_a, cell_b) {
            ...
            if volume_before_a > volume_before_b {
                ...
                assert!(volume_decrease > 10.0, "Source volume should decrease by at least 10.0 units, decreased by: {}", volume_decrease);
                assert!(volume_increase > 10.0, "Dest volume should increase by at least 10.0 units, increased by: {}", volume_increase);
                assert!((volume_decrease - volume_increase).abs() < 1e-6, "Volume transfer should be equal: decrease={}, increase={}", volume_decrease, volume_increase);
            }
            ...
            assert!((total_volume_before - total_volume_after).abs() < 1e-6, "Volume should be conserved");
            assert!((total_energy_before - total_energy_after).abs() < 1e-6, "Energy should be conserved");
        } else {
