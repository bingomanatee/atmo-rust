...
use crate::constants::{
    ANOMALY_DECAY_RATE, ANOMALY_ENERGY_AMOUNT, ANOMALY_SPAWN_CHANCE, ANOMALY_VOLUME_AMOUNT,
    AVG_STARTING_VOLUME_KM_3, CELL_JOULES_EQUILIBRIUM, CELL_JOULES_START, LEVEL_AMT,
};
...
impl AsthSimLinked {
    ...
    pub fn run_step(&mut self) {
        ...
        echo!(self, "  📏 Leveling cells...");
        // self.level_cells();

        echo!(self, "  🌀 Processing anomalies...");
        self.process_anomalies();

        echo!(self, "  ⚡ Spawning new anomalies...");
        self.spawn_anomalies();
        
        echo!(self, "  🔥 Cooling cells...");
        self.cool_cells();

        echo!(self, "  ⏭️  Advancing to next step...");
        self.advance_to_next_step();

        // Export visualization every 5 frames if enabled
        if self.visualize && self.current_step % self.vis_freq == 0 {
            echo!(self, "  🖼️  Exporting visualization...");
    ...
    fn export_visualization(&self) {
        ...
        echo!(
            self,
            "    🎨 Creating PNG exporter ({}x {})...",
            width,
            height
        );
        // Create PNG exporter with moderate resolution for faster rendering
        let mut exporter = PngExporter::new(width, height, self.planet.clone());
        exporter.set_debug(self.debug);

        echo!(self, "    🖌️  Rendering Voronoi image...");
        let image = exporter.render_voronoi_image_from_cells(&cells_for_export);

        echo!(self, "    💾 Saving image file...");
        // Save the image
        let filename = format!("{}/step_{:04}.png", output_dir, self.current_step);
        match image.save(&filename) {
            Ok(_) => {
                echo!(self, "    ✅ Exported visualization: {}", filename);
            }
            Err(e) => eprintln!(
    ...
    fn level_between_neighbors(
        ...
        if volume_diff > 0.0 {
            ...
            let base_transfer = volume_diff/2.0 ;
            let transfer_amount = base_transfer * LEVEL_AMT;
        ...
        for neighbor_id in neighbors {
            if let Some(neighbor_cell) = self.cells.get(&neighbor_id) {
                ...
                if let Some(neighbor_next) = &neighbor_current.next {
                    ...
                    neighbor_next_mut.cell.anomaly_energy +=
                        ANOMALY_ENERGY_AMOUNT * 0.5 * neighbor_energy_sign;
                    neighbor_next_mut.cell.anomaly_volume +=
                        ANOMALY_VOLUME_AMOUNT * 0.5 * neighbor_volume_sign;
                }
            }
        }
    }
    ...
    pub fn run_simulation(&mut self, steps: u64) {
        ...
        echo!(self, "📊 Initial state: {} cells loaded", self.cells.len());

        for step in 1..=steps {
            echo!(self, "🔄 Running step {} of {}:", step, steps);
            self.run_step();
            ...
            if self.visualize && step % 5 == 0 {
                echo!(
                    self,
                    "🖼️  Exported visualization for step {}",
                    self.current_step
                );
            }
        }
        ...
        if self.visualize {
            ...
            echo!(self, "🎉 Simulation completed!");
        }
    }
}
